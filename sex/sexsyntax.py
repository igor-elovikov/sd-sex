...
class SexHighlighter(QSyntaxHighlighter):
    ...
    keywords = [
        ...
        "and",
        "or",
        sexparser.export_function_name,
        sexparser.declare_inputs_function_name,
        "for",
        "in",
        "endfor",
        "endif",
        ...
        "return",
        "def",
        sexparser.sequence_function_name,
        sexparser.setvar_function_name,
    ]
    ...
    def setup_rules(self, functions, builtin_functions, builtin_types):
        ...
        rules += [(r"\b%s\b" % w, 0, STYLES["keyword"]) for w in SexHighlighter.keywords]

        rules += [(r"\b%s\b" % w, 0, STYLES["function"]) for w in functions]

        rules += [(r"\b%s\b" % w, 0, STYLES["builtin_function"]) for w in builtin_functions]

        rules += [
            (r"\b%s\b" % w, 0, STYLES["builtin_constant"]) for w in SexHighlighter.builtin_constant
        ]

        rules += [(r"\b%s\b" % w, 0, STYLES["builtin_type"]) for w in builtin_types]

        rules += [(r"\b%s\b" % sexparser.output_variable_name, 0, STYLES["output"])]

        rules += [(r"\B\$%s\b" % w[1:], 0, STYLES["output"]) for w in system_inputs]

        rules += [(r"\b(f|f2|f3|f4|i|i2|i3|i4|b|s)\$(\w+)\b", 0, STYLES["output"])]

        rules += [(r"%s" % o, 0, STYLES["operator"]) for o in SexHighlighter.operators]

        rules += [(r"%s" % b, 0, STYLES["brace"]) for b in SexHighlighter.braces]
    ...
    def highlightBlock(self, text):
        ...
        for expression, nth, format in self.rules:
            ...
            while match.hasNext():
                ...
                index = match_result.capturedStart(nth)
                length = match_result.capturedLength(nth)
